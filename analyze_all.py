...
for i, row in df_clean.iterrows():
    texts.append(plt.text(row['PC1'], row['PC2'], row['車種名'], fontsize=8, alpha=0.9))
adjust_text(texts, arrowprops=dict(arrowstyle="-", color='gray', lw=0.5, alpha=0.6))
plt.title('PCA 車種のポジショニングマップ (引き出し線付き)')
plt.xlabel(f'PC1 ({pca.explained_variance_ratio_[0]:.2%} Variance)')
plt.ylabel(f'PC2 ({pca.explained_variance_ratio_[1]:.2%} Variance)')
plt.grid(True)
plt.savefig(f'{out_dir}/pca_map.png', dpi=200, bbox_inches='tight')
plt.close()
...
for i, feature in enumerate(eval_cols):
    texts.append(plt.text(loadings[i, 0], loadings[i, 1], feature, fontsize=10, color='indigo'))
    plt.arrow(0, 0, loadings[i, 0]*0.95, loadings[i, 1]*0.95, color='purple', alpha=0.15, head_width=0.015)
adjust_text(texts, arrowprops=dict(arrowstyle="->", color='purple', lw=0.6, alpha=0.6))
...
plt.grid(True)
plt.savefig(f'{out_dir}/pca_variables_map.png', dpi=200, bbox_inches='tight')
plt.close()
...
for i, txt in enumerate(row_coords.index):
    texts.append(plt.text(row_coords.iloc[i, 0], row_coords.iloc[i, 1], txt, fontsize=8, alpha=0.9, color='blue'))
for i, txt in enumerate(col_coords.index):
    texts.append(plt.text(col_coords.iloc[i, 0], col_coords.iloc[i, 1], txt, fontsize=12, color='darkred', weight='bold'))
adjust_text(texts, arrowprops=dict(arrowstyle="-", color='gray', lw=0.4, alpha=0.7))
plt.title('Correspondence Analysis Map (Car Name vs Category)')
plt.legend()
plt.grid(True)
plt.savefig(f'{out_dir}/correspondence_map.png', dpi=200, bbox_inches='tight')
plt.close()
...
for i, txt in enumerate(row_coords3.index):
    texts.append(plt.text(row_coords3.iloc[i, 0], row_coords3.iloc[i, 1], txt, fontsize=8, alpha=0.9, color='green'))
for i, txt in enumerate(col_coords3.index):
    texts.append(plt.text(col_coords3.iloc[i, 0], col_coords3.iloc[i, 1], txt, fontsize=12, color='darkorange', weight='bold'))
adjust_text(texts, arrowprops=dict(arrowstyle="-", color='gray', lw=0.4, alpha=0.7))
plt.title('Quantification Theory III Map (Car Name vs Segment)')
plt.legend()
plt.grid(True)
plt.savefig(f'{out_dir}/quant3_map.png', dpi=200, bbox_inches='tight')
plt.close()
...
texts = []
for i, feature in enumerate(eval_cols):
    texts.append(plt.text(imp_t[i], ach_t[i], feature, fontsize=10, alpha=0.9))
adjust_text(texts, arrowprops=dict(arrowstyle="-", color='teal', lw=0.5, alpha=0.7))
...
plt.savefig(f'{out_dir}/cs_portfolio_items.png', dpi=200, bbox_inches='tight')
plt.close()

# 9. CS Portfolio (120 Cars)
...
for i, txt in enumerate(df_clean['車種名']):
    texts.append(plt.text(score_t[i], cp_t[i], txt, fontsize=8, alpha=0.8))
# adjust_text takes some time for 120 elements, force iterations constraint if needed
adjust_text(texts, arrowprops=dict(arrowstyle="-", color='gray', lw=0.4, alpha=0.6), max_iterations=2000)
...
plt.grid(True, alpha=0.3)
plt.savefig(f'{out_dir}/cs_portfolio_cars.png', dpi=200, bbox_inches='tight')
plt.close()

print("All analyses completed with adjustText bounding logic.")
